...
async function resolvePlayerId(pool, val) {
  if (!val) return null;
  ...
  // Fallback to first player if available so attendance record is never lost
...
export async function createAttendance(req, res) {
  try {
    ...
    const pool = await getPool();
    const resolvedPlayerId = await resolvePlayerId(pool, player_id);
    if (!resolvedPlayerId) {
